        ...
        try:
            ...
            if now-ln>=300:
                ...
                if c and NEWS_CHANNEL_ID:
                    try: await bot.send_message(NEWS_CHANNEL_ID,f"📰 Новости {c['country']}: стабильность и развитие")
                    except: pass
                ln=now
            if now-lb>=30:
...
async def on_startup(dp):
    ...
    await bot.set_my_commands([
        BotCommand("start","🔄 Меню"),BotCommand("edit_money","💰 Казна"),
        BotCommand("edit_speed","⏱️ Скорость")])
