...
class DB:
    ...
    def init(self):
        ...
        self.ex("CREATE TABLE IF NOT EXISTS wars(id INTEGER PRIMARY KEY AUTOINCREMENT,attacker_id INTEGER,defender_id INTEGER,target_city_id INTEGER,start_time REAL,peace_offer INTEGER DEFAULT 0,passage_through INTEGER)")
        ...
        await msg.answer(f"🎮 {p['country']} | {p['government']}\n💰 ${STARTING_MONEY:,}\nСтатус: {ps}",
            reply_markup=colored_kb(
                [("💰 Экономика","eco"),("🏙️ Города","cities"),("🤝 Альянсы","all"),
                 ("🛡️ Техника","tech"),("🌍 Обзор мира","world"),("⏯️ Пауза","pause_menu"),("⚙️ Настройки","settings")],
                [("⚔️ Война","war"),("👥 Армия","army"),("🚶 Проход войск","passage"),("🏳️ Мир","peace_opt")]))
    else:
...
async def world(cb: types.CallbackQuery):
    players = db.ex("SELECT country,government FROM players WHERE in_game=1").fetchall()
    wars = db.ex("SELECT COUNT(*) as c FROM wars").fetchone()['c']
    txt = f"🌍 ОБЗОР МИРА\n\n⚔️ Активных войн: {wars}\n\n📋 Страны:\n"
    for p in players[:20]:
        txt += f"• {p['country']} ({p['government']})\n"
...
async def passage_menu(cb: types.CallbackQuery):
    await cb.message.edit_text("🚶 ЗАПРОС ПРОХОДА ВОЙСК\n\nВыберите страну, через которую хотите пройти:",
        reply_markup=kb([(f"🚶 {t['country']}",f"pass_{t['user_id']}") for t in db.ex(
            "SELECT user_id,country FROM players WHERE in_game=1 AND user_id!=?",(cb.from_user.id,)).fetchall()[:10]]
            +[("↩️ Назад","back")],1))
...
async def passage_ask_target(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("🎯 Выберите цель атаки (к кому идёте):",
        reply_markup=kb([(f"🎯 {t['country']}",f"passtarget_{t['user_id']}") for t in db.ex(
            "SELECT user_id,country FROM players WHERE in_game=1 AND user_id!=? AND user_id!=?",
            (cb.from_user.id,through_id)).fetchall()[:10]]+[("↩️ Назад","back")],1))
...
async def passage_send(cb: types.CallbackQuery, state: FSMContext):
    ...
    db.ex("INSERT INTO passage_requests(from_user_id,to_user_id,target_user_id,created_at) VALUES(?,?,?,?)",
          (cb.from_user.id,through_id,target_id,time.time()))
    db.com()
    through_country = db.ex("SELECT country FROM players WHERE user_id=?",(through_id,)).fetchone()['country']
    try: await bot.send_message(through_id,
        f"🚶 {cb.from_user.first_name} просит проход войск через {through_country} для атаки!\n"
        f"Ответьте:",reply_markup=kb([("✅ Разрешить",f"passok_{cb.from_user.id}_{target_id}"),
                                       ("❌ Отказать",f"passno_{cb.from_user.id}")]))
    except: pass
...
async def passage_ok(cb: types.CallbackQuery):
    ...
    db.ex("UPDATE passage_requests SET status='approved' WHERE from_user_id=? AND to_user_id=? AND target_user_id=?",
          (from_id,cb.from_user.id,target_id))
    db.com()
    try: await bot.send_message(from_id,"✅ Проход разрешён! Можете атаковать.")
    except: pass
    await cb.message.edit_text("✅ Вы разрешили проход!",reply_markup=kb([("↩️ В меню","back")]))
...
async def passage_no(cb: types.CallbackQuery):
    ...
    db.ex("UPDATE passage_requests SET status='rejected' WHERE from_user_id=? AND to_user_id=?",
          (from_id,cb.from_user.id))
    db.com()
    try: await bot.send_message(from_id,"❌ В проходе отказано!")
    except: pass
    await cb.message.edit_text("❌ Вы отказали в проходе.",reply_markup=kb([("↩️ В меню","back")]))
...
async def edit_money(msg: types.Message):
    try:
        ...
        db.com()
        await msg.answer(f"✅ Стартовая казна: ${v:,.0f}")
...
async def edit_speed(msg: types.Message):
    try:
        v = int(msg.get_args())
        if v not in [4,12,24,48,168]: await msg.answer("❌ 4/12/24/48/168 часов"); return
            ...
            if now-le>=6:
                ...
                for c in cs:
                    db.ex("UPDATE cities SET treasury=treasury+?,population=population+?,last_update=? WHERE id=?",
                          (c['population']*100,int(c['population']*0.01),now,c['id']))
                db.com(); le=now
            if now-ln>=300:
                c = db.ex("SELECT country FROM players WHERE in_game=1 ORDER BY RANDOM() LIMIT 1").fetchone()
                if c and NEWS_CHANNEL_ID:
                    news_list = [
                        f"📈 Экономика {c['country']} показывает рекордный рост!",
                        f"🏗️ В {c['country']} запущен масштабный инфраструктурный проект",
                        f"🤝 {c['country']} заключает важное торговое соглашение",
                        f"🎭 В столице {c['country']} открылся международный фестиваль",
                        f"🔬 Учёные {c['country']} совершили научный прорыв",
                        f"🌱 {c['country']} переходит на зелёную энергетику",
                        f"🏆 Спортсмены {c['country']} победили на Олимпиаде",
                        f"💻 {c['country']} запускает программу цифровизации",
                        f"🎓 Университет {c['country']} вошёл в топ-100 мира",
                        f"🚀 {c['country']} планирует космическую миссию",
                        f"🏥 В {c['country']} открылась современная больница",
                        f"🎨 Выставка искусства {c['country']} покоряет мир",
                        f"📡 {c['country']} запустила новый спутник связи",
                        f"🌍 {c['country']} лидирует по экологическим показателям",
                        f"💰 Инвесторы проявляют интерес к экономике {c['country']}"
                    ]
                    try: await bot.send_message(NEWS_CHANNEL_ID, random.choice(news_list))
                    except: pass
                ln=now
            if now-lb>=30:
...
async def on_startup(dp):
    ...
    await bot.set_my_commands([
        BotCommand("start","🔄 Меню"),BotCommand("reset","🔄 Сброс"),
        BotCommand("edit_money","💰 Казна"),BotCommand("edit_speed","⏱️ Скорость")])
